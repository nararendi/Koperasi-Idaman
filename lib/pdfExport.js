...
function openPrintPDF(documentHtml, documentTitle) {
...
export const pdfExport = {
  ...
  exportDaftarTagihanPDF(tagihanData, settings, labelPeriode) {
    ...
    const rowsHtml = (tagihanData.list || []).map((row) => `
      <tr>
        ...
        <td style="text-align: right; border: 1px solid #000; padding: 4px 6px;">${row.wajib > 0 ? `Rp ${fmt(row.wajib)}` : ''}</td>
        <td style="text-align: right; border: 1px solid #000; padding: 4px 6px;">${row.sukarela > 0 ? `Rp ${fmt(row.sukarela)}` : ''}</td>
        <td style="text-align: right; border: 1px solid #000; padding: 4px 6px;">${row.qurban > 0 ? `Rp ${fmt(row.qurban)}` : ''}</td>
        <td style="text-align: center; border: 1px solid #000; padding: 4px; font-weight: bold;">${row.cicilanKe || ''}</td>
        <td style="text-align: right; border: 1px solid #000; padding: 4px 6px;">${row.pokok > 0 ? `Rp ${fmt(row.pokok)}` : ''}</td>
        <td style="text-align: right; border: 1px solid #000; padding: 4px 6px;">${row.jasa > 0 ? `Rp ${fmt(row.jasa)}` : ''}</td>
        <td style="text-align: right; border: 1px solid #000; padding: 4px 6px;">${row.sembako > 0 ? `Rp ${fmt(row.sembako)}` : ''}</td>
        <td style="text-align: right; border: 1px solid #000; padding: 4px 6px; font-weight: bold; color: #dc2626;">Rp ${fmt(row.jumlah)}</td>
      </tr>
    `).join('');
    ...
    const html = `
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 8pt; line-height: 1.2;">
        <thead>
          <tr>
            <th rowspan="2" style="background: #111; color: #fff; border: 1px solid #000; padding: 6px 3px; width: 30px; text-align: center;">NO.</th>
            <th rowspan="2" style="background: #111; color: #fff; border: 1px solid #000; padding: 6px 3px; width: 65px; text-align: center;">NO. ANGGOTA</th>
            <th rowspan="2" style="background: #111; color: #fff; border: 1px solid #000; padding: 6px 6px; text-align: center;">NAMA</th>
            <th colspan="3" style="background: #111; color: #fff; border: 1px solid #000; padding: 4px; text-align: center;">SIMPANAN</th>
            <th colspan="4" style="background: #111; color: #fff; border: 1px solid #000; padding: 4px; text-align: center;">POTONGAN</th>
            <th rowspan="2" style="background: #111; color: #fff; border: 1px solid #000; padding: 6px 6px; width: 85px; text-align: center;">JUMLAH</th>
          </tr>
          <tr>
            <th style="background: #111; color: #fff; border: 1px solid #000; padding: 4px; width: 68px; text-align: center;">WAJIB</th>
            <th style="background: #111; color: #fff; border: 1px solid #000; padding: 4px; width: 68px; text-align: center;">SUKARELA</th>
            <th style="background: #111; color: #fff; border: 1px solid #000; padding: 4px; width: 68px; text-align: center;">QURBAN</th>
            <th style="background: #111; color: #fff; border: 1px solid #000; padding: 4px; width: 45px; text-align: center;">CICILAN KE</th>
            <th style="background: #111; color: #fff; border: 1px solid #000; padding: 4px; width: 75px; text-align: center;">POKOK</th>
            <th style="background: #111; color: #fff; border: 1px solid #000; padding: 4px; width: 68px; text-align: center;">JASA</th>
            <th style="background: #111; color: #fff; border: 1px solid #000; padding: 4px; width: 70px; text-align: center;">SEMBAKO</th>
          </tr>
        </thead>
        <tbody>
          ${rowsHtml}
          <tr style="font-weight: bold; background: #fafafa;">
            <td colspan="3" style="text-align: center; border: 1px solid #000; padding: 6px; font-weight: 900; font-size: 8.5pt;">JUMLAH</td>
            <td style="text-align: right; border: 1px solid #000; padding: 6px; font-weight: 900;">Rp ${fmt(totals.wajib)}</td>
            <td style="text-align: right; border: 1px solid #000; padding: 6px; font-weight: 900;">Rp ${fmt(totals.sukarela)}</td>
            <td style="text-align: right; border: 1px solid #000; padding: 6px; font-weight: 900;">Rp ${fmt(totals.qurban)}</td>
            <td style="border: 1px solid #000; padding: 6px;"></td>
            <td style="text-align: right; border: 1px solid #000; padding: 6px; font-weight: 900;">Rp ${fmt(totals.pokok)}</td>
            <td style="text-align: right; border: 1px solid #000; padding: 6px; font-weight: 900;">Rp ${fmt(totals.jasa)}</td>
            <td style="text-align: right; border: 1px solid #000; padding: 6px; font-weight: 900;">Rp ${fmt(totals.sembako)}</td>
            <td style="text-align: right; border: 1px solid #000; padding: 6px; font-weight: 900; color: #dc2626; font-size: 8.5pt;">Rp ${fmt(totals.total)}</td>
          </tr>
        </tbody>
      </table>

      <!-- Tanda Tangan Resmi Sesuai Gambar -->
      <div style="margin-top: 35px; width: 100%;">
        <table style="width: 100%; border: none; font-size: 9pt; text-align: left;">
          <tr>
            <td style="width: 55%; vertical-align: top; padding-left: 20px;">
              Ketua Koperasi
              <div style="height: 60px;"></div>
              <strong>${settings.ketua || 'Asep Solehudin'}</strong>
            </td>
            <td style="width: 45%; vertical-align: top;">
              ${dateLine}<br/>
              Bendahara Koperasi
              <div style="height: 60px;"></div>
              <strong>${settings.bendahara || 'Ica Cahyani'}</strong>
            </td>
          </tr>
        </table>
      </div>
    ...
    openPrintPDF(html, `Daftar_Tagihan_Koperasi_${labelPeriode || 'Bulan'}`);
  }
};
