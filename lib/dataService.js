...
const initialData = {
  ...
  kas: [],
  sembako_produk: [
    { id: 'PRD-001', nama: 'Beras Premium Ramos 5 Kg', kategori: 'Beras', satuan: 'Karung 5kg', harga_beli: 65000, harga_jual: 74000, stok: 35 },
    { id: 'PRD-002', nama: 'Minyak Goreng Refill 2 Liter', kategori: 'Minyak', satuan: 'Pouch 2L', harga_beli: 30000, harga_jual: 34500, stok: 48 },
    { id: 'PRD-003', nama: 'Gula Pasir Kristal Putih 1 Kg', kategori: 'Gula', satuan: 'Bungkus 1kg', harga_beli: 15000, harga_jual: 17500, stok: 50 },
    { id: 'PRD-004', nama: 'Tepung Terigu Segitiga Biru 1 Kg', kategori: 'Tepung', satuan: 'Bungkus 1kg', harga_beli: 11000, harga_jual: 13000, stok: 40 },
    { id: 'PRD-005', nama: 'Telur Ayam Negeri 1 Kg (Tray)', kategori: 'Telur', satuan: 'Kg', harga_beli: 26000, harga_jual: 29000, stok: 25 }
  ],
  sembako_transaksi: [],
  qurban_peserta: [],
  qurban_mutasi: [],
...
function getDB() {
  ...
  try {
    ...
    return {
      ...
      sembako_produk: Array.isArray(parsed.sembako_produk) && parsed.sembako_produk.length > 0 ? parsed.sembako_produk.filter(Boolean) : initialData.sembako_produk,
...
export const dataService = {
  ...
  async fetchFromSupabase() {
    ...
    try {
      const [
        { data: anggotaData, error: errAnggota },
        { data: simpananData },
        { data: pinjamanData },
        { data: angsuranData },
        { data: kasData },
        { data: settingsData },
        { data: sembakoProdukData },
        { data: sembakoTrxData },
        { data: qurbanPesertaData },
        { data: qurbanMutasiData },
        { data: tagihanOverrideData }
      ] = await Promise.all([
        client.from('anggota').select('*'),
        client.from('simpanan').select('*'),
        client.from('pinjaman').select('*'),
        client.from('riwayat_angsuran').select('*'),
        client.from('kas').select('*'),
        client.from('settings').select('*').limit(1),
        client.from('sembako_produk').select('*'),
        client.from('sembako_transaksi').select('*'),
        client.from('qurban_peserta').select('*'),
        client.from('qurban_mutasi').select('*'),
        client.from('tagihan_override').select('*')
      ]);
      ...
      if (sembakoProdukData && sembakoProdukData.length > 0) {
      ...
      if (sembakoTrxData) {
        db.sembako_transaksi = sembakoTrxData.map((st) => ({
          ...
          nama_pembeli: st.nama_pembeli,
          items: typeof st.items === 'string' ? JSON.parse(st.items) : st.items,
          total_belanja: Number(st.total_belanja),
          metode_bayar: st.metode_bayar,
          status_bayar: st.status_bayar
        }));
      }

      if (qurbanPesertaData) {
      ...
      if (tagihanOverrideData && tagihanOverrideData.length > 0) {
        const overrides = {};
  ...
  updateSembakoProduk(id, updatedData) {
    const db = getDB();
    const idx = db.sembako_produk.findIndex((p) => p.id === id);
    if (idx !== -1) {
  ...
  deleteSembakoProduk(id) {
    const db = getDB();
    db.sembako_produk = db.sembako_produk.filter((p) => p.id !== id);
    saveDB(db);
  },
  ...
  addSembakoTransaksi({ pembeli, nomor_anggota = '', items = [], total, bayar, kembali, metode = 'Tunai' }) {
    const db = getDB();
    const today = new Date().toISOString().split('T')[0];
    const newTx = {
    ...
    // Kurangi stok barang
    items.forEach((item) => {
      const prod = db.sembako_produk.find((p) => p.id === item.id);
      if (prod) {
