...
export const dataService = {
  ...
  payPinjamanInstallment({ pinjamanId, jumlahBayar, metode = 'Tunai', penerima = 'Admin Kasir', angsuranKe = null, pokok = null, bunga = null }) {
    const db = getDB();
    const item = db.pinjaman.find((p) => p.id === pinjamanId || p.nomor_pinjaman === pinjamanId);
    if (!item) return null;
    ...
    const newKas = {
      ...
      jumlah: payAmount,
      keterangan: `Angsuran ke-${actualAngsuranKe} Pinjaman ${item.nama} (${item.nomor_pinjaman})`,
      ref_id: angsId
    };
    db.kas.unshift(newKas);
  ...
  async addQurbanPeserta({ nama, nomor_anggota = '', tipe_hewan, target_nominal, tahun_qurban = '1448 H / 2026' }) {
    const db = getDB();
    const today = new Date().toISOString().split('T')[0];
