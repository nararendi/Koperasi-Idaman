  ...
  pinjaman: [
    {
      ...
      status: 'Berjalan', // Diajukan, Disetujui, Berjalan, Lunas, Ditolak
...
export const dataService = {
  ...
  updatePinjamanStatus(pinjamanId, newStatus) {
    ...
    // If changing to 'Berjalan' (Disbursement/Pencairan Dana) and was not 'Berjalan' before
  ...
  getLaporanData(periodeBulan = '', periodeTahun = '') {
    const db = getDB();
    ...
    const filterFn = (itemDate) => {
      ...
      if (periodeTahun && !itemDate.startsWith(periodeTahun)) return false;
      if (periodeBulan && itemDate.slice(5, 7) !== periodeBulan) return false;
      return true;
    };
    ...
    // SHU Estimation (Pendapatan Jasa Bunga + Admin - Biaya Operasional)
    let totalBungaTerkumpul = 0;
    pinjamanList.forEach((p) => {
      if (p.riwayat_angsuran && p.riwayat_angsuran.length > 0) {
        totalBungaTerkumpul += p.riwayat_angsuran.length * (p.angsuran_bunga || 0);
      }
    });
    ...
    return {
      arusKas: {
        ...
        saldoKasBersih: kasSummary.saldo
      },
      neraca: {
