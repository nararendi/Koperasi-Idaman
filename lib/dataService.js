...
function getDB() {
  ...
  try {
    ...
    return {
      ...
      kas: parsed.kas || [],
      sembako_produk: parsed.sembako_produk && parsed.sembako_produk.length > 0 ? parsed.sembako_produk : initialData.sembako_produk,
      sembako_transaksi: parsed.sembako_transaksi || [],
      qurban_peserta: parsed.qurban_peserta || [],
      qurban_mutasi: parsed.qurban_mutasi || [],
...
export const dataService = {
  ...
  getTagihanBulanan(monthStr = '') {
    ...
    const anggotaList = db.anggota || [];
    const list = anggotaList.map((a, idx) => {
      const memberId = a.nomor_anggota || a.id;
      ...
      (db.simpanan || []).forEach((s) => {
        if ((s.nomor_anggota === memberId || s.nama_anggota === a.nama_lengkap || s.nama_anggota === a.nama) &&
      ...
      (db.qurban_mutasi || []).forEach((q) => {
        if ((q.nomor_anggota === memberId || q.nama_peserta === a.nama_lengkap || q.nama_peserta === a.nama) &&
      ...
      const activeLoan = (db.pinjaman || []).find((p) => 
        (p.nomor_anggota === memberId || p.nama === a.nama_lengkap || p.nama === a.nama) && p.status === 'Berjalan'
      );
      ...
      (db.sembako_transaksi || []).forEach((st) => {
        if ((st.nomor_anggota === memberId || st.pembeli === a.nama_lengkap || st.pembeli === a.nama) &&
