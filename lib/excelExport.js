...
export const excelExport = {
  ...
  exportAnggota(anggotaList, settings) {
    ...
    const rowsHtml = (anggotaList || []).map((a, idx) => `
      <tr>
        <td class="center">${idx + 1}</td>
  ...
  exportPinjaman(pinjamanList, summary, settings) {
    ...
    const rowsHtml = (pinjamanList || []).map((p, idx) => `
      <tr>
        <td class="center">${idx + 1}</td>
        <td class="center font-mono"><strong>${p.nomor_pinjaman || p.id}</strong></td>
        <td class="center">${p.tanggal}</td>
        <td class="bold">${p.nama}</td>
        <td class="right">${fmt(p.jumlah)}</td>
        <td class="center">${p.tenor} Bln (${p.bunga}%)</td>
        <td class="right bold">${fmt(p.total_angsuran_bulanan)}</td>
        <td class="right">${fmt(p.total_pinjaman)}</td>
        <td class="right">${fmt(p.total_terbayar)}</td>
        <td class="right bold ${p.sisa_hutang > 0 ? 'bg-danger' : 'bg-success'}">${fmt(p.sisa_hutang)}</td>
        <td class="center bold">${p.status}</td>
      </tr>
    `).join('');
  ...
  exportDaftarTagihanExcel(tagihanData, settings, labelPeriode) {
    ...
    const rowsHtml = (tagihanData.list || []).map((row) => `
      <tr>
        ...
        <td>${row.nama}</td>
        <td class="right">${row.wajib > 0 ? fmt(row.wajib) : ''}</td>
        <td class="right">${row.sukarela > 0 ? fmt(row.sukarela) : ''}</td>
        <td class="right">${row.qurban > 0 ? fmt(row.qurban) : ''}</td>
        <td class="center bold">${row.cicilanKe || ''}</td>
        <td class="right">${row.pokok > 0 ? fmt(row.pokok) : ''}</td>
        <td class="right">${row.jasa > 0 ? fmt(row.jasa) : ''}</td>
        <td class="right">${row.sembako > 0 ? fmt(row.sembako) : ''}</td>
        <td class="right bold bg-danger" style="color: #9f1239;">${fmt(row.jumlah)}</td>
      </tr>
    `).join('');
    ...
    const content = `
      <table>
        <tr>
          <td colspan="11" class="kop-title" style="font-size: 14pt;">DAFTAR TAGIHAN ${(settings.namaKoperasi || 'KOPERASI GURU KARYAWAN SMK ASSALAAM BANDUNG').toUpperCase()}</td>
        </tr>
        <tr>
      ...
      <table>
        <thead>
          <tr>
            <th rowspan="2" style="background: #111; color: #fff; width: 45px;">NO.</th>
            <th rowspan="2" style="background: #111; color: #fff; width: 90px;">NO. ANGGOTA</th>
            <th rowspan="2" style="background: #111; color: #fff; width: 220px;">NAMA</th>
            <th colspan="3" style="background: #111; color: #fff;">SIMPANAN</th>
            <th colspan="4" style="background: #111; color: #fff;">POTONGAN</th>
            <th rowspan="2" style="background: #111; color: #fff; width: 130px;">JUMLAH</th>
          </tr>
          <tr>
            <th style="background: #111; color: #fff; width: 95px;">WAJIB</th>
            <th style="background: #111; color: #fff; width: 95px;">SUKARELA</th>
            <th style="background: #111; color: #fff; width: 95px;">QURBAN</th>
            <th style="background: #111; color: #fff; width: 65px;">CICILAN KE</th>
            <th style="background: #111; color: #fff; width: 100px;">POKOK</th>
            <th style="background: #111; color: #fff; width: 95px;">JASA</th>
            <th style="background: #111; color: #fff; width: 100px;">SEMBAKO</th>
          </tr>
        </thead>
        <tbody>
          ${rowsHtml}
          <tr>
            <td colspan="3" class="center bold" style="font-size: 11pt; background: #e2e8f0;">JUMLAH</td>
            <td class="right bold" style="background: #e2e8f0;">${fmt(totals.wajib)}</td>
            <td class="right bold" style="background: #e2e8f0;">${fmt(totals.sukarela)}</td>
            <td class="right bold" style="background: #e2e8f0;">${fmt(totals.qurban)}</td>
            <td class="center bold" style="background: #e2e8f0;"></td>
            <td class="right bold" style="background: #e2e8f0;">${fmt(totals.pokok)}</td>
            <td class="right bold" style="background: #e2e8f0;">${fmt(totals.jasa)}</td>
            <td class="right bold" style="background: #e2e8f0;">${fmt(totals.sembako)}</td>
            <td class="right bold bg-danger" style="font-size: 11pt; color: #9f1239;">${fmt(totals.total)}</td>
          </tr>
        </tbody>
      </table>
      ...
      <table class="ttd-table">
        <tr>
          <td colspan="5" style="width: 50%; text-align: left; padding-left: 20px;">Ketua Koperasi</td>
        ...
        <tr>
          <td colspan="5" style="text-align: left; padding-left: 20px;"><strong>${settings.ketua || 'Asep Solehudin'}</strong></td>
          <td colspan="6" style="text-align: left; padding-left: 20px;"><strong>${settings.bendahara || 'Ica Cahyani'}</strong></td>
        </tr>
      </table>
    `;
